...
function GenomicContent() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error(e); 
      toast.error(e.userMessage || t.genomic.loadFailed);
    }
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      toast.success(t.genomic.addSuccess);
      setForm({ gene_name: '', snp_id: '', genotype: '', effect_score: '', trait_description: '' });
      fetchData();
    } catch (e) { 
      console.error(e); 
      toast.error(e.userMessage || t.genomic.addFailed);
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      toast.success(t.genomic.analysisSuccess);
    } catch (e) { 
      console.error(e); 
      toast.error(e.userMessage || t.genomic.analysisFailed);
    }
    finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-slate-600">{t.genomic.subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">{t.genomic.geneName}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">{t.genomic.snpId}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">{t.genomic.genotype}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">{t.genomic.effectScore}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">{t.genomic.traitDescription}</label>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-900">{t.genomic.yourVariants} ({data.length})</h2>
            ...
            {loading ? (
              ...
              <p className="text-sm text-slate-500 py-8 text-center">{t.genomic.noData}</p>
            )}
          </div>
        </div>
        ...
        {analysis && (
          <div className="mt-6 bg-white rounded-xl border border-slate-200 p-6">
            <h2 className="text-lg font-semibold text-slate-900 mb-4">{t.genomic.analysisResults}</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-slate-50 rounded-lg">
                <p className="text-sm text-slate-500">{t.genomic.totalSnps}</p>
                <p className="text-2xl font-bold text-slate-900">{analysis.total_snps}</p>
              </div>
              <div className="p-4 bg-slate-50 rounded-lg">
                <p className="text-sm text-slate-500">{t.genomic.keyGenes}</p>
                <p className="text-lg font-medium text-slate-900">{analysis.key_genes.join(', ')}</p>
              </div>
              <div className="p-4 bg-slate-50 rounded-lg">
                <p className="text-sm text-slate-500">{t.genomic.nutritionRisks}</p>
